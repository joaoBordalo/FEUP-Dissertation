	...
	for (int s = startVertex; s < endVertex; s++)
	{
		...
		//Single source shortest-paths problem
		{
			...
			while (!Q.empty())
			{
				...
				if (isInStack[v] == false)
				{
					...
					list<Edge*> adjEdges = graph->GetAdjacentVertices(v);
		...
		//Accumulation
		{
			...
			while (S.size() != 0)
			{
				...
				if (w != s)
				{
					if(w== 75 || w== 78 || w== 116 || w== 117 || w== 128 || w==129 || w==130 || w==143 || w==250 || w==423 || w==432 || w==485 || w==486 || w==487 || w==488 || w==493)
					{
						if(betweenness[w] != 0 )
						{
							values.push_back(make_pair(w,betweenness[w]));
						}
					}
